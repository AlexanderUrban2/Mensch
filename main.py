...
def init_dice(dice):
    for i in range(0, 6):
        ...
        dice[i] = pygame.transform.smoothscale(dice[i], (screen_width//11, screen_height//11))
    return dice

def get_gamefield_distance_to_border():
    return (screen_width*1.04827 - screen_width)/2

def get_gamefield_size():
...
def roll_dice(dice):
    for i in range(0, 10):
        ...
        screen.blit(dice[number], ((screen_width - image_width)//2, (screen_height - image_heigth)//2))
        time.sleep(0.005)
    return number
...
def start_game(keys):
    ...
    screen.blit(myfont.render('Press Space to roll the Dice', False, (0, 0, 0)), ((screen_width - text_width)//2, int(screen_height*0.9)))
    figure.set_figures()
    pygame.display.update()
    while run:
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:
                    ...
                    if number+1 == 6:
                        ...
                        screen.blit(myfont.render('U can Move ' + str(number +1) + ' Steps now', False, (0, 0, 0)), ((screen_width - text_width)//2, int(screen_height*0.9)))
                        pygame.display.update()
...
gamefield_width = get_gamefield_size()
gamefield_height = get_gamefield_size()
...
text_width, text_heigth = myfont.size("Press X to start the Game")
screen.blit(myfont.render('Press X to start the Game', False, (255, 0, 0)), ((screen_width - text_width)//2, (screen_height - text_heigth)//2))
